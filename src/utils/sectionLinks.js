...
async function setFrameIndex(section) {
    ...
    if (!section) {
        ...
        if (section === "#firstProjectWrapper") window.scroll({top: window.innerHeight, behavior: "smooth"});
        else document.querySelector(section).scrollIntoView({ behavior: "smooth" })
        // let top = section === "#firstProjectWrapper"
        //     ? window.innerHeight
        //     : document.querySelector(section).getBoundingClientRect().top - window.innerHeight * 0.1
